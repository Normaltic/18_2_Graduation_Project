...
/*		console.log(nextProps.selectedPage);
	...
	onMouseDown(e) {
		...
		setTimeout( () => {
			if( this.longTouchFlag ) this.props.handleMiniToolMenuSetVisible({x: pos[0], y: pos[1]});
			this.longTouchFlag = false;
		}, 800);
	...
	onMouseMove(e) {
		let pos = this.getEventPosition(e);
		if( this.lastMousePosition[0] != pos[0] || this.lastMousePosition[1] != pos[1] ) this.longTouchFlag = false;
	...
	onMouseUp(e) {
		if( this.longTouchFlag ) this.longTouchFlag = false;
		if( this.drawingStatus ) {
			this.drawingStatus = false;
